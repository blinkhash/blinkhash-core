Blinkhash Core version 0.12.0 is now available from:
...
backwards-compatible with pre-0.10 versions of Blinkhash Core or other software:
...
pre-0.12 versions of Blinkhash Core or other software.

If you want to downgrade after you have done a reindex with 0.12.0 or later,
you will need to reindex when you first start Blinkhash Core version 0.11 or
earlier.
...
ECDSA signatures inside Blinkhash transactions now use validation using
...
Previous versions of Blinkhash Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Blinkhash Core was to raise the default minimum
relay fee.

Blinkhash Core 0.12 will have a strict maximum size on the mempool. The
...
Blinkhash Core 0.12 also introduces new default policy limits on the length and
...
Blinkhash Core 0.12 nodes. Blinkhash Core will only allow replacement of
...
Note that the wallet in Blinkhash Core 0.12 does not yet have support for
...
This allows running blinkhashd without having to do any manual configuration.
...
Blinkhash Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Blinkhash Core will relay transactions with
insufficient fees depending on the setting of `-limitfreerelay=<r>` (default:
`r=15` kB per minute) and `-blockprioritysize=<s>`.

In Blinkhash Core 0.12, when mempool limit has been reached a higher minimum
...
entirely in Blinkhash Core 0.13, and it is at this time undecided whether the
...
Blinkhash Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Blinkhash Core automatically creates a hidden service to listen on, without
manual configuration. Blinkhash Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Blinkhash Core is listening, and
...
Blinkhashd can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Blinkhash
Core to use floating fees. By default, Blinkhash Core will use floating
fees.
...
(default: `0.0002` BLKH/kB).

At all times, Blinkhash Core will cap fees at `-maxtxfee=<x>` (default:
0.10) BLKH.
Furthermore, Blinkhash Core will never create transactions paying less than
...
`blinkhash.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
Then, to tunnel a SSL connection on 28332 to a RPC server bound on localhost on port 19854 do:

    stunnel -d 28332 -r 127.0.0.1:19854 -p stunnel.pem -P ''
...
caching. A sample config for apache2 could look like:
    ...
    <Location /blinkhashrpc>
        ProxyPass http://127.0.0.1:9854/
        ProxyPassReverse http://127.0.0.1:9854/
        # optional enable digest auth
        # AuthType Digest
        # ...

        # optional bypass blinkhashd rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< blinkhashrpc:<password>
    </Location>
...
Restarting blinkhashd will no longer clear out the list of banned peers; instead
...
- #6214 `87406aa` [OSX] revert renaming of Blinkhash-Qt.app and use CFBundleDisplayName (partial revert of #6116) (Jonas Schnelli)
- #6218 `9d67b10` build/gitian misc updates (Cory Fields)
- #6269 `d4565b6` gitian: Use the new blinkhash-detached-sigs git repo for OSX signatures (Cory Fields)
...
- #6600 `ef4945f` Include blinkhash-tx binary on Debian/Ubuntu (Zak Wilcox)
...
- #6390 `6a73d66` tests: Fix blinkhash-tx signing test case (Wladimir J. van der Laan)
...
- #6730 `cb4d6d0` build: Remove dependency of blinkhash-cli on secp256k1 (Wladimir J. van der Laan)
...
- #7209 `f3ad812` test: don't override BLINKHASHD and BLINKHASHCLI if they're set (Wladimir J. van der Laan)
...
- #6387 `11576a5` [blinkhash-cli] improve error output (Jonas Schnelli)
- #6401 `6db53b4` Add BLINKHASHD_SIGTERM_TIMEOUT to OpenRC init scripts (Florian Schmaus)
- #6430 `b01981e` doc: add documentation for shared library libblinkhashconsensus (Braydon Fuller)
...
- #6504 `b6fee6b` Rationalize currency unit to "BLKH" (Ross Nicoll)
