Blinkhash Core version 0.13.0 is now available from:
...
- Adding `dbcache=100` in blinkhash.conf
...
blinkhash-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/blinkhash-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Blinkhash Core code base has
...
HD wallets are incompatible with older versions of Blinkhash Core.
...
proposed parameters, a future release of Blinkhash Core is expected that
...
The two phases are distinct in the Blinkhash-Qt GUI. During the first one,
...
- #7550 `8b958ab` Input-from-stdin mode for blinkhash-cli (laanwj)
...
- #8153 `75ec320` `fundrawtransaction` feeRate: Use BLKH/kB (MarcoFalke)
...
- #7613 `3798e5d` Add autocomplete to blinkhash-qt's console window (GamerSg)
...
- #7636 `fb0ac48` Add blinkhash address label to request payment QR code (makevoid)
- #8231 `760a6c7` Fix a bug where the SplashScreen will not be hidden during startup (jonasschnelli)
- #8256 `af2421c` BUG: blinkhash-qt crash (fsb4000)
- #8257 `ff03c50` Do not ask a UI question from blinkhashd (sipa)
- #8288 `91abb77` Network-specific example address (laanwj)
- #7707 `a914968` UI support for abandoned transactions (jonasschnelli)
- #8207 `f7a403b` Add a link to the Blinkhash-Core repository and website to the About Dialog (MarcoFalke)
...
- #8047 `37f9a1f` Test_framework: Set wait-timeout for blinkhashd procs (MarcoFalke)
...
- #8164 `0f24eaf` [Blinkhash-Tx] fix missing test fixtures, fix 32bit atoi issue (jonasschnelli)
...
- #7744 `d8e862a` test_framework: detect failure of blinkhashd startup (laanwj)
...
- #7975 `fc23fee` Update blinkhash-core GitHub links (MarcoFalke)
