...
Blinkhash Core version 0.20.0 is now available from:
...
- Instead of running `make` simply, you should instead run
  `BLINKHASH_GENBUILD_NO_GIT=1 make`.
...
#### Removal of BIP61 reject network messages from Blinkhash Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since Blinkhash Core version 0.18.0.
...
- Testing or debugging of implementations of the Blinkhash P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of Blinkhash Core. Blinkhash Core logs debug messages
...
- OpenSSL is no longer used by Blinkhash Core.  (#17265)

- BIP70 support has been fully removed from Blinkhash Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  `blinkhashd help` for more information.  This option is experimental and
...
- All custom settings configured when Blinkhash Core starts are now
...
- The "Start Blinkhash Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://bitcoincore.org/en/releases/0.18.0/#wallet-gui)
  ...
  affected anyone who simultaneously used multiple Blinkhash Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed Blinkhash Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/bitcoin-core/HWI) for
  signing.  Future versions of Blinkhash Core should support a GUI option
...
- Blinkhash Core's automatically-generated source code documentation is
...
- The `blinkhash-cli` utility used with the `-getinfo` parameter now
...
- The `blinkhash-cli` utility now accepts a `-stdinwalletpassphrase`
...
- Command line options prefixed with main/test/regtest network names like
  `-main.port=9855` `-test.server=1` previously were allowed but ignored. Now
...
- #17068 Always generate `blinkhashstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_blinkhash-qt` in msvc build (sipsorcery)
...
- #17910 Remove double `LIBBLINKHASH_SERVER` linking (fanquake)
...
- #18397 Fix libevent linking for `bench_blinkhash` binary (hebasto)
- #18426 scripts: `Previous_release`: improve behaviour on failed download (theStack)
- #18429 Remove double `LIBBLINKHASH_SERVER` from bench-Makefile (brakmic)
...
- #18702 Fix ASLR for blinkhash-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate blinkhash schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest blinkhash.conf (sdaftuar)
...
- #18022 Fix appveyor `test_blinkhash` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (blinkhash-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone blinkhash-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about Blinkhash Core files (hebasto)
...
- #17119 Fix broken blinkhash-cli examples (andrewtoth)
...
- #17503 Remove bitness from blinkhash-qt help message and manpage (laanwj)
...
- #17648 Rename wallet-tool references to blinkhash-wallet (hel-o)
...
- #18341 Replace remaining literal BLKH with `CURRENCY_UNIT` (domob1812)
...
- #16115 On blinkhashd startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from blinkhash-cli -getinfo (malevolent)
