...
Blinkhash Core version 0.19.1 is now available from:
...
From Blinkhash Core 0.17.0 onwards, macOS versions earlier than 10.10 are no
longer supported, as Blinkhash Core is now built using Qt 5.9.x which requires
macOS 10.10+. Additionally, Blinkhash Core does not yet change appearance when
...
- #17488 fix "blinkhashd already running" warnings on macOS (fanquake)
