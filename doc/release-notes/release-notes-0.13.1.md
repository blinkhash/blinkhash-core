Blinkhash Core version 0.13.1 is now available from:
...
updates will be released anymore. Without security updates, using a blinkhash
wallet on a XP machine is irresponsible at least.

In addition to that, with 0.12.x there have been varied reports of Blinkhash Core
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Blinkhash wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Blinkhash.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Blinkhash block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of blinkhashs they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of blinkhashs being spent is exactly
...
- **Increased security for multisig:** Blinkhash addresses (both P2PKH addresses
  ...
  believed to be provided by Blinkhash's choice of parameters for its Elliptic
  Curve Digital Security Algorithm [ECDSA].)

- **More efficient almost-full-node security** Satoshi Nakamoto's original
  Blinkhash paper describes a method for allowing newly-started full nodes to
  skip downloading and validating some data from historic blocks that are
  protected by large amounts of proof of work.  Unfortunately, Nakamoto's
  method can't guarantee that a newly-started node using this method will
  produce an accurate copy of Blinkhash's current ledger (called the UTXO set),
  ...
  note that Blinkhash Core does not provide an option to use this capability as
  of this 0.13.1 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Blinkhash users to individually opt-in to almost any change in the Blinkhash
  Script language when those users receive new transactions.  Features
  currently being researched by Blinkhash Core contributors that may use this
  capability include support for Schnorr signatures, which can improve the
  privacy and efficiency of multisig transactions (or transactions with
  multiple inputs), and Merklized Abstract Syntax Trees (MAST), which can
  improve the privacy and efficiency of scripts with two or more conditions.
  Other Blinkhash community members are studying several other improvements
...
Since Blinkhash Core 0.10.0, nodes have defaulted to only relaying and
...
- `blinkhash-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `blinkhash-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
...
- #8848 `7a34a46` Add NULLDUMMY verify flag in blinkhashconsensus.h (jl2012)
...
- #8940 `5b4192b` Add x9 service bit support to dnsseed.bluematt.me, seed.blinkhashstats.com (TheBlueMatt, cdecker)
- #8944 `685e4c7` Remove bogus assert on number of oubound connections. (TheBlueMatt)
- #8949 `0dbc48a` Be more agressive in getting connections to peers with relevant services (gmaxwell)

### Build system
- #8293 `fa5b249` Allow building libblinkhashconsensus without any univalue (luke-jr)
- #8492 `8b0bdd3` Allow building bench_blinkhash by itself (luke-jr)
...
- #8829 `2a8bca4` Add blinkhash-tx JSON tests (jnewbery)
- #8834 `1dd1783` blockstore: Switch to dumb dbm (MarcoFalke)
- #8835 `d87227d` nulldummy.py: Don't run unused code (MarcoFalke)
- #8836 `eb18cc1` blinkhash-util-test.py should fail if the output file is empty (jnewbery)
...
- #8860 `0bee740` util: Move wait_blinkhashds() into stop_nodes() (MarcoFalke)
...
- #8683 `8a7d7ff` Fix incorrect file name blinkhash.qrc  (blinkhashsSG)
...
- #8817 `bcf3806` update blinkhash-tx to output witness data (jnewbery)
