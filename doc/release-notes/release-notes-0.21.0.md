...
Blinkhash Core version 0.21.0 is now available from:
...
using the Linux kernel, macOS 10.12+, and Windows 7 and newer.  Blinkhash
Core should also work on most other Unix-like systems but is not as
frequently tested on them.  It is not recommended to use Blinkhash Core on
unsupported systems.

From Blinkhash Core 0.20.0 onwards, macOS versions earlier than 10.12 are no
longer supported. Additionally, Blinkhash Core does not yet change appearance
...
- This release adds support for Tor version 3 hidden services, and rumoring them
  ...
  Version 2 hidden services are still fully supported by Blinkhash Core, but the
...
- The Tor onion service that is automatically created by setting the
  ...
  `-datadir`) and can be removed if not needed. Blinkhash Core will no longer
...
- A new `blinkhash-cli -netinfo` command provides a network peer connections
...
- A new `blinkhash-cli -generate` command, equivalent to RPC `generatenewaddress`
  followed by `generatetoaddress`, can generate blocks for command line testing
  purposes. This is a client-side version of the former `generate` RPC. See the
  help for details. (#19133)

- The `blinkhash-cli -getinfo` command now displays the wallet name and balance for
  each of the loaded wallets when more than one is loaded (e.g. in multiwallet
  mode) and a wallet is not specified with `-rpcwallet`. (#18594)

- The `connections` field of `blinkhash-cli -getinfo` is now expanded to return a JSON
...
- The `startupnotify` option is used to specify a command to
  execute when Blinkhash Core has finished with its startup
  sequence. (#15367)
...
- The `-salvagewallet` startup option has been removed. A new `salvage` command
  has been added to the `blinkhash-wallet` tool which performs the salvage
...
Blinkhash Core will no longer automatically create new wallets on startup. It will
load existing wallets specified by `-wallet` options on the command line or in
`blinkhash.conf` or `settings.json` files. And by default it will also load a
top-level unnamed ("") wallet. However, if specified wallets don't exist,
Blinkhash Core will now just log warnings instead of creating new wallets with
new keys and addresses like previous releases did.

New wallets can be created through the GUI (which has a more prominent create
wallet option), through the `blinkhash-cli createwallet` or `blinkhash-wallet
...
Users who already have a Blinkhash Core wallet can continue to use it as they did before without
any change in behavior. Newly created Legacy Wallets (which remains the default type of wallet) will
behave as they did in previous versions of Blinkhash Core.
...
The change to using descriptors changes the default derivation paths used by Blinkhash Core
...
- A new `fee_rate` parameter/option denominated in satoshis per vbyte (sat/vB)
  ...
  `walletcreatefundedpsbt` still exists for setting a fee rate in BLKH per 1,000
  vbytes (BLKH/kvB), but it is expected to be deprecated soon to avoid
  confusion. For these RPCs, the fee rate error message is updated from BLKH/kB
  to sat/vB and the help documentation in BLKH/kB is updated to BLKH/kvB. The
  `send` and `sendtoaddress` RPC examples are updated to aid users in creating
  transactions with explicit fee rates. (#20305, #11413)

- The `bumpfee` RPC `fee_rate` option is changed from BLKH/kvB to sat/vB and the
  ...
  difference between BLKH/kvB and sat/vB units means that a transaction with a
  fee rate mistakenly calculated in BLKH/kvB rather than sat/vB should raise an
...
- Wallets created or loaded in the GUI will now be automatically loaded on
  startup, so they don't need to be manually reloaded next time Blinkhash Core is
  started. The list of wallets to load on startup is stored in
  `\<datadir\>/settings.json` and augments any command line or `blinkhash.conf`
...
- #19284 Add seed.blinkhash.wiz.biz to DNS seeds (wiz)
...
- #20198 Show name, format and if uses descriptors in blinkhash-wallet tool (jonasschnelli)
...
- #19133 rpc, cli, test: add blinkhash-cli -generate command (jonatack)
...
- #20055 Set HTTP Content-Type in blinkhash-cli (laanwj)
...
### Build system
- #18504 Drop blinkhash-tx and blinkhash-wallet dependencies on libevent (ryanofsky)
...
- #18653 add coverage for blinkhash-cli -rpcwait (jonatack)
...
- #18739 Document how to fuzz Blinkhash Core using Honggfuzz (practicalswift)
...
- #19061 Add link to Visual Studio build readme (maitreblinkhash)
...
- #18817 Document differences in blinkhashd and blinkhash-qt locale handling (practicalswift)
