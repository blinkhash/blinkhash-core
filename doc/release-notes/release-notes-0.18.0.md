Blinkhash Core version 0.18.0 is now available from:
...
`/Applications/Blinkhash-Qt` (on Mac) or `blinkhashd`/`blinkhash-qt` (on
Linux).
...
recommended to use Blinkhash Core on unsupported systems.

Blinkhash Core should also work on most other Unix-like systems but is not
as frequently tested on them.

From 0.17.0 onwards, macOS <10.10 is no longer supported. 0.17.0 is
built using Qt 5.9.x, which doesn't support versions of macOS older than
10.10. Additionally, Blinkhash Core does not yet change appearance when
...
`contrib/init/blinkhashd.service`, it has been changed to use
`/var/lib/blinkhashd` as the data directory instead of
`~blinkhash/.blinkhash`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/blinkhashd` will
exist has enough space (check using `df -h /var/lib/blinkhashd`), and
...
- The `rpcallowip` option can no longer be used to automatically listen
  ...
  [...] -p 127.0.0.1:9854:9854` (this is an extra `:9854` over the
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://bitcoincore.org/en/contact/) the Blinkhash Core
...
The systemd init file (`contrib/init/blinkhashd.service`) has been changed
to use `/var/lib/blinkhashd` as the data directory instead of
`~blinkhash/.blinkhash`. This change makes Blinkhash Core more consistent with
...
- A new [document](https://github.com/bitcoin/bitcoin/blob/master/doc/blinkhash-conf.md)
  about the `blinkhash.conf` file describes how to use it to configure
  Blinkhash Core.

- A new document introduces Blinkhash Core's BIP174 [Partially-Signed
  Blinkhash Transactions
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Blinkhash-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Blinkhash Core to libssl vulnerabilities in the past, builders who don't
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Blinkhash Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Blinkhash Core, subsequently importing
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart blinkhashd with the `-deprecatedrpc=generate`
...
- A new `blinkhash-wallet` tool is now distributed alongside Blinkhash
...
This section describes planned changes to Blinkhash Core that may affect
other Blinkhash software and services.

- Since version 0.16.0, Blinkhash Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Blinkhash Core 0.20 (expected about
  a year after 0.18), Blinkhash Core will default to native segwit
  ...
  to bech32 addresses, and if the Blinkhash Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Blinkhash Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any Blinkhash Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Blinkhash Core
...
- A sub-project of Blinkhash Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Blinkhash Core.  See their [project
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Blinkhash Core's own implementation, although entropy
  gathered by Blinkhash Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Blinkhash Core a
...
- On macOS, Blinkhash Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `blinkhash_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libblinkhash_server.a` (jnewbery)
...
- #13926 [Tools] blinkhash-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like blinkhashd (MarcoFalke)
...
- #14521 qt, docs: Fix `blinkhash-qt -version` output formatting (hebasto)
...
- #11625 Add BlinkhashApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libblinkhash_server` (laanwj)
...
- #15154 Configure: blinkhash-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/blinkhash-wallet (Sjors)
...
- #11526 Visual Studio build configuration for Blinkhash Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libblinkhash_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in blinkhash-util-test.py (isghe)
...
- #12775 Integration of property based testing into Blinkhash Core (Christewart)
...
- #12246 Bugfix: Only run blinkhash-tx tests when blinkhash-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_blinkhash` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_blinkhash` (MarcoFalke)
...
- #14186 blinkhash-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_blinkhash.conf` message on startup if conf file exists (leishman)
...
- #12255 Update blinkhash.service to conform to init.md (dongcarl)
...
- #14497 Add doc/blinkhash-conf.md (hebasto)
...
- #15354 Add missing `blinkhash-wallet` tool manpages (MarcoFalke)
...
- #15765 correct blinkhashconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support blinkhash folder on OSX (JimmyMow)
