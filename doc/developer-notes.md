...
- [Development guidelines](#development-guidelines)
    - [General Blinkhash Core](#general-blinkhash-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Blinkhash Core uses [Doxygen](https://www.doxygen.nl/) to generate its official documentation.
...
with "play blinkhashs" on a test network.
...
Blinkhash Core is a multi-threaded application, and deadlocks or other
...
```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_blinkhash
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_blinkhash --log_level=test_suite
$ valgrind -v --leak-check=full src/blinkhashd -printtoconsole
...
# A coverage report will now be accessible at `./test_blinkhash.coverage/index.html`.
```
...
To profile a running blinkhashd process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep blinkhashd` -- sleep 60
```
...
Blinkhash Core can be compiled with various "sanitizers" enabled, which add
...
- [Main thread (`blinkhashd`)](https://doxygen.bitcoincore.org/bitcoind_8cpp.html#a0ddf1224851353fc92bfbff6f499fa97)
  : Started from `main()` in `blinkhashd.cpp`. Responsible for starting up and
...
- Net threads:
  ...
  - [ThreadSocketHandler (`b-net`)](https://doxygen.bitcoincore.org/class_c_connman.html#a765597cbfe99c083d8fa3d61bb464e34)
    : Sends/Receives data from peers on port 9855.
...
However, in open source software such as Blinkhash Core, where everyone uses
...
pay attention to for reviewers of Blinkhash Core code.

General Blinkhash Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: Blinkhash Core uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
- Use include guards to avoid the problem of double inclusion. The header file
  `foo/bar.h` should use the include guard identifier `BLINKHASH_FOO_BAR_H`, e.g.

```c++
#ifndef BLINKHASH_FOO_BAR_H
#define BLINKHASH_FOO_BAR_H
...
#endif // BLINKHASH_FOO_BAR_H
```
...
Some of these are maintained by active developers of Blinkhash Core, in which case
...
a Blinkhash Core subtree, so that they can be integrated quickly. Cosmetic changes
...
file descriptors, it will cause problems with Blinkhash's `select()` loop, because
...
compatibility between nodes. This happened in Blinkhash 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to Blinkhash's
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `blinkhash-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `blinkhash-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Blinkhash Core, the wallet was always
