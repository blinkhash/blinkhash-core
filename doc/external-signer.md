# Support for signing transactions outside of Blinkhash Core

Blinkhash Core can be launched with `-signer=<cmd>` where `<cmd>` is an external tool which can sign transactions and perform other functions. For example, it can be used to communicate with a hardware wallet.

## Example usage

The following example is based on the [HWI](https://github.com/bitcoin-core/HWI) tool. Version 2.0 or newer is required. Although this tool is hosted under the Blinkhash Core GitHub organization and maintained by Bitcoin Core developers, it should be used with caution. It is considered experimental and has far less review than Blinkhash Core itself. Be particularly careful when running tools such as these on a computer with private keys on it.

When using a hardware wallet, consult the manufacturer website for (alternative) software they recommend. As long as their software conforms to the standard below, it should be able to work with Blinkhash Core.

Start Blinkhash Core:

```sh
$ blinkhashd -signer=../HWI/hwi.py
```
...
```
$ blinkhash-cli enumeratesigners
{
  "signers": [
    {
...
```sh
$ blinkhash-cli createwallet "hww" true true "" true true true
```
...
```sh
$ blinkhash-cli -rpcwallet=<wallet> getnewaddress
$ blinkhash-cli -rpcwallet=<wallet> walletdisplayaddress <address>
```
...
Under the hood this uses a [Partially Signed Blinkhash Transaction](psbt.md).

```sh
$ blinkhash-cli -rpcwallet=<wallet> sendtoaddress <address> <amount>
```
...
In order to be compatible with Blinkhash Core any signer command should conform to the specification below. This specification is subject to change. Ideally a BIP should propose a standard so that other wallets can also make use of it.

Prerequisite knowledge:
* [Output Descriptors](descriptors.md)
* Partially Signed Blinkhash Transaction ([PSBT](psbt.md))
...
## How Blinkhash Core uses the Signer API
