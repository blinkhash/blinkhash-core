...
Below are some notes on how to build Blinkhash Core for Windows.

The options known to work for building Blinkhash Core on Windows are:
...
Note that for WSL the Blinkhash Core source path MUST be somewhere in the default mount file system, for
example /usr/src/blinkhash, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\blinkhash`, for example:

    make install DESTDIR=/mnt/c/workspace/blinkhash
...
It's not possible to build the Blinkhash Core code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the Blinkhash Core source code).
