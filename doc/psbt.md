# PSBT Howto for Blinkhash Core

Since Blinkhash Core 0.17, an RPC interface exists for Partially Signed Blinkhash
...
PSBT is an interchange format for Blinkhash transactions that are not fully signed
...
#### Multisig with multiple Blinkhash Core instances
...
Blinkhash Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `blinkhash-cli` is used.
...
Later, when *V* BLKH has been received on *Amulti*, and Bob and Carol want to
