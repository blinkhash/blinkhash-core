...
The Blinkhash Core daemon can be configured to act as a trusted "border
router", implementing the blinkhash wire protocol and relay, making
...
The ZeroMQ feature in Blinkhash Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building blinkhashd:
...
These options can also be provided in blinkhash.conf.
...
From the perspective of blinkhashd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into blinkhashd directly. Furthermore, no information is
...
using. Blinkhashd appends an up-counting sequence number to each
