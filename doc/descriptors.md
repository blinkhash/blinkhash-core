# Support for Output Descriptors in Blinkhash Core

Since Blinkhash Core v0.17, there is support for Output Descriptors. This is a
...
- Followed by the actual key, which is either:
  ...
  - [WIF](https://en.blinkhash.it/wiki/Wallet_import_format) encoded private keys may be specified instead of the corresponding public key, with the same meaning.
...
on Blinkhash's OP_CHECKMULTISIG opcode. To support these, we introduce the
...
transactions related to the multisig. Lastly, it is not recommended to use anything other than a Blinkhash Core descriptor wallet to serve as your
...
existing Blinkhash Core wallets, a convenience function `combo` is
...
All RPCs in Blinkhash Core will include the checksum in their output. Only
