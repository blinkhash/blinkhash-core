# User-space, Statically Defined Tracing (USDT) for Blinkhash Core

Blinkhash Core includes statically defined tracepoints to allow for more
...
## Adding tracepoints to Blinkhash Core
...
Multiple tools can list the available tracepoints in a `blinkhashd` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Blinkhash Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/blinkhashd
stap net        outbound_message 0x0000000000107c05 /src/blinkhashd
stap validation block_connected  0x00000000002fb10c /src/blinkhashd
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Blinkhash Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/blinkhashd | grep NT_STAPSDT -A 4 -B 2
