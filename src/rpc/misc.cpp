...
static RPCHelpMan validateaddress()
{
    return RPCHelpMan{"validateaddress",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The blinkhash address to validate"},
                },
                RPCResult{
                    RPCResult::Type::OBJ, "", "",
                    {
                        {RPCResult::Type::BOOL, "isvalid", "If the address is valid or not"},
                        {RPCResult::Type::STR, "address", /* optional */ true, "The blinkhash address validated"},
...
static RPCHelpMan verifymessage()
{
    return RPCHelpMan{"verifymessage",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The blinkhash address to use for the signature."},
...
static RPCHelpMan echo(const std::string& name)
{
    return RPCHelpMan{name,
                ...
                "blinkhash-cli and the GUI. There is no server-side difference.",
                {
    ...
    return RPCHelpMan{
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue {
            ...
            if (interfaces::Ipc* ipc = local_init.ipc()) {
                // Spawn a new blinkhash-node process and call makeEcho to get a
                // client pointer to a interfaces::Echo instance running in
                // that process. This is just for testing. A slightly more
                // realistic test spawning a different executable instead of
                // the same executable would add a new blinkhash-echo executable,
                // and spawn blinkhash-echo below instead of blinkhash-node. But
                // using blinkhash-node avoids the need to build and install a
                // new executable just for this one test.
                auto init = ipc->spawnProcess("blinkhash-node");
                echo = init->makeEcho();
                ipc->addCleanup(*echo, [init = init.release()] { delete init; });
            } else {
                // IPC support is not available because this is a blinkhashd
                // process not a blinkhashd-node process, so just create a local
