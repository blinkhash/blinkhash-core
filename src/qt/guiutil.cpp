...
#include <qt/blinkhashaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Blinkhash address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new BlinkhashAddressEntryValidator(parent));
    widget->setCheckValidator(new BlinkhashAddressCheckValidator(parent));
}
...
bool parseBlinkhashURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no blinkhash: URI
    if(!uri.isValid() || uri.scheme() != QString("blinkhash"))
        return false;
...
bool parseBlinkhashURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseBlinkhashURI(uriInstance, out);
}

QString formatBlinkhashURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("blinkhash:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;
...
bool openBlinkhashConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", BLINKHASH_CONF_FILENAME));
    ...
    /* Open blinkhash.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Blinkhash.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Blinkhash (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Blinkhash (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for Blinkhash*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "blinkhash.desktop";
    return GetAutostartDir() / strprintf("blinkhash-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a blinkhash.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Blinkhash\n";
        else
            optionFile << strprintf("Name=Blinkhash (%s)\n", chain);
