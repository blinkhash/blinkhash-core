...
#include <qt/blinkhashamountfield.h>
...
uint256 SendCoins(CWallet& wallet, SendCoinsDialog& sendCoinsDialog, const CTxDestination& address, CAmount amount, bool rbf)
{
    ...
    entry->findChild<BlinkhashAmountField*>("payAmount")->setValue(amount);
...
//
//     QT_QPA_PLATFORM=xcb     src/qt/test/test_blinkhash-qt  # Linux
//     QT_QPA_PLATFORM=windows src/qt/test/test_blinkhash-qt  # Windows
//     QT_QPA_PLATFORM=cocoa   src/qt/test/test_blinkhash-qt  # macOS
void TestGUI(interfaces::Node& node)
{
...

    {
        ...
        QString balanceComparison = BlinkhashUnits::formatWithUnit(unit, balance, false, BlinkhashUnits::SeparatorStyle::ALWAYS);
    ...
    // Amount input
    BlinkhashAmountField* amountInput = receiveCoinsDialog.findChild<BlinkhashAmountField*>("reqAmount");
    ...
    for (QWidget* widget : QApplication::topLevelWidgets()) {
        if (widget->inherits("ReceiveRequestDialog")) {
            ...
            QCOMPARE(uri.count("blinkhash:"), 2);
...
void WalletTests::walletTests()
{
    ...
    if (QApplication::platformName() == "minimal") {
        ...
        QWARN("Skipping WalletTests on mac build with 'minimal' platform set due to Qt bugs. To run AppTests, invoke "
              "with 'QT_QPA_PLATFORM=cocoa test_blinkhash-qt' on mac, or else use a linux or windows build.");
        return;
    }
#endif
    TestGUI(m_node);
