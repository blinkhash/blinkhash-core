This directory contains integration tests that test blinkhashd and its
...
blinkhashd and blinkhash-qt by interacting with them through the RPC and P2P
interfaces.
- [util](/test/util) which tests the utilities (blinkhash-util, blinkhash-tx, ...).
...
Before tests can be run locally, Blinkhash Core must be built.  See the [building instructions](/doc#building) for help.
...
The P2P and RPC ports used by the blinkhashd nodes-under-test are chosen to make
conflicts with other processes unlikely. However, if there is another blinkhashd
process running on the system (perhaps from a previous test which hasn't successfully
killed all its blinkhashd nodes), then there may be a port conflict which will
cause the test to fail. It is recommended that you run the tests on a system
where no other blinkhashd processes are running.

On linux, the test framework will warn if there is another
blinkhashd process running when the tests are started.

If there are zombie blinkhashd processes after test failure, you can kill them
by running the following commands. **Note that these commands will kill all
blinkhashd processes running on the system, so should not be used if any non-test
blinkhashd processes are being run.**
...
sure blinkhashd processes are stopped as above):
...
- when run by [our CI (Continuous Integration)](/ci/README.md), no logs are output to the console. However, if a test
  fails, the `test_framework.log` and blinkhashd `debug.log`s will all be dumped
...
`test_framework.log` and blinkhashd `debug.log`s can be combined into a single
...
call methods that interact with the blinkhashd nodes-under-test.

If further introspection of the blinkhashd instances themselves becomes
...
```bash
cat /tmp/user/1000/testo9vsdjo3/node1/regtest/blinkhashd.pid
```

Then you can use the pid to start `gdb`:

```bash
gdb /home/example/blinkhashd <pid>
```
