...
class WalletTest(BlinkhashTestFramework):
    ...
    def run_test(self):
        ...
        if not self.options.descriptors:
            # Tests legacy watchonly behavior which is not present (and does not need to be tested) in descriptor wallets
            assert_equal(self.nodes[0].getbalances()['mine']['trusted'], 5000)
            assert_equal(self.nodes[0].getwalletinfo()['balance'], 5000)
            assert_equal(self.nodes[1].getbalances()['mine']['trusted'], 5000)

            assert_equal(self.nodes[0].getbalances()['watchonly']['immature'], 500000)
            assert 'watchonly' not in self.nodes[1].getbalances()
        ...
        assert_equal(self.nodes[0].getbalance("*"), 5000)
        assert_equal(self.nodes[0].getbalance("*", 1), 5000)
        assert_equal(self.nodes[0].getbalance(minconf=1), 5000)
        if not self.options.descriptors:
            assert_equal(self.nodes[0].getbalance(minconf=0, include_watchonly=True), 10000)
            assert_equal(self.nodes[0].getbalance("*", 1, True), 10000)
        else:
            assert_equal(self.nodes[0].getbalance(minconf=0, include_watchonly=True), 5000)
        ...
        self.sync_all()
        txs = create_transactions(self.nodes[1], self.nodes[0].getnewaddress(), 6000, [Decimal('0.01'), Decimal('0.02')])
        ...
        def test_balances(*, fee_node_1=0):
            ...
            expected_balances_0 = {'mine':      {'immature':          Decimal('0E-8'),
                                                 'trusted':           Decimal('999.99'),  # change from node 0's send
                                                 'untrusted_pending': Decimal('6000.0')},
                                   'watchonly': {'immature':          Decimal('500000'),
                                                 'trusted':           Decimal('5000.0'),
                                                 'untrusted_pending': Decimal('0E-8')}}
            expected_balances_1 = {'mine':      {'immature':          Decimal('0E-8'),
                                                 'trusted':           Decimal('0E-8'),  # node 1's send had an unsafe input
                                                 'untrusted_pending': Decimal('3000.0') - fee_node_1}}  # Doesn't include output of node 0's send since it was spent
            if self.options.descriptors:
                del expected_balances_0["watchonly"]
            assert_equal(self.nodes[0].getbalances(), expected_balances_0)
            assert_equal(self.nodes[1].getbalances(), expected_balances_1)
            # getbalance without any arguments includes unconfirmed transactions, but not untrusted transactions
            assert_equal(self.nodes[0].getbalance(), Decimal('999.99'))  # change from node 0's send
            assert_equal(self.nodes[1].getbalance(), Decimal('0'))  # node 1's send had an unsafe input
            # Same with minconf=0
            assert_equal(self.nodes[0].getbalance(minconf=0), Decimal('999.99'))
            ...
            assert_equal(self.nodes[0].getunconfirmedbalance(), Decimal('6000'))  # output of node 1's spend
            assert_equal(self.nodes[1].getunconfirmedbalance(), Decimal('3000') - fee_node_1)  # Doesn't include output of node 0's send since it was spent
            # getwalletinfo.unconfirmed_balance
            assert_equal(self.nodes[0].getwalletinfo()["unconfirmed_balance"], Decimal('6000'))
            assert_equal(self.nodes[1].getwalletinfo()["unconfirmed_balance"], Decimal('3000') - fee_node_1)
        ...
        balance_node0 = Decimal('6999.99')  # node 1's send plus change from node 0's send
        balance_node1 = Decimal('2999.98')  # change from node 0's send
        ...
        txs = create_transactions(self.nodes[1], self.nodes[0].getnewaddress(), Decimal('2999.97'), [Decimal('0.01')])
        self.nodes[1].sendrawtransaction(txs[0]['hex'])
        ...
        for _ in range(3):
            ...
            txid = self.nodes[0].sendtoaddress(self.nodes[0].getnewaddress(), 9999)
