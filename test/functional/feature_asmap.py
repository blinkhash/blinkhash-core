...
Verify node behaviour and debug log when launching blinkhashd in these cases:

1. `blinkhashd` with no -asmap arg, using /16 prefix for IP bucketing

2. `blinkhashd -asmap=<absolute path>`, using the unit test skeleton asmap

3. `blinkhashd -asmap=<relative path>`, using the unit test skeleton asmap

4. `blinkhashd -asmap/-asmap=` with no file specified, using the default asmap

5. `blinkhashd -asmap` restart with an addrman containing new and tried entries

6. `blinkhashd -asmap` with no file specified and a missing default asmap file

7. `blinkhashd -asmap` with an empty (unparsable) default asmap file
...
class AsmapTest(BlinkhashTestFramework):
    ...
    def fill_addrman(self, node_id):
        ...
        for addr, tried in [[0, True], [1, False]]:
            self.nodes[node_id].addpeeraddress(address=f"101.{addr}.0.0", tried=tried, port=9855)

    def test_without_asmap_arg(self):
        self.log.info('Test blinkhashd with no -asmap arg passed')
        self.stop_node(0)
        with self.node.assert_debug_log(['Using /16 prefix for IP bucketing']):
            self.start_node(0)

    def test_asmap_with_absolute_path(self):
        self.log.info('Test blinkhashd -asmap=<absolute path>')
        self.stop_node(0)
    ...
    def test_asmap_with_relative_path(self):
        self.log.info('Test blinkhashd -asmap=<relative path>')
        self.stop_node(0)
        name = 'ASN_map'
    ...
    def test_asmap_interaction_with_addrman_containing_entries(self):
        self.log.info("Test blinkhashd -asmap restart with addrman containing new and tried entries")
        self.stop_node(0)
        shutil.copyfile(self.asmap_raw, self.default_asmap)
    ...
    def test_default_asmap_with_missing_file(self):
        self.log.info('Test blinkhashd -asmap with missing default map file')
        self.stop_node(0)
        msg = f"Error: Could not find asmap file \"{self.default_asmap}\""
        self.node.assert_start_raises_init_error(extra_args=['-asmap'], expected_msg=msg)

    def test_empty_asmap(self):
        self.log.info('Test blinkhashd -asmap with empty map file')
        self.stop_node(0)
