...
# TestP2PConn: A peer we use to send messages to blinkhashd, and store responses.
...
class CompactBlocksTest(BlinkhashTestFramework):
    ...
    # This test actually causes blinkhashd to (reasonably!) disconnect us, so do this last.
    ...
    # Test that blinkhashd requests compact blocks when we announce new blocks
    ...
    def test_incorrect_blocktxn_response(self, test_node):
        ...
        # Note that it's possible for blinkhashd to be smart enough to know we're
    ...
    def test_getblocktxn_handler(self, test_node):
        version = test_node.cmpct_version
        node = self.nodes[0]
        # blinkhashd will not send blocktxn responses for blocks whose height is
