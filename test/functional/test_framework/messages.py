...
"""Blinkhash test framework primitive and message structures

CBlock, CTransaction, CBlockHeader, CTxIn, CTxOut, etc....:
    data structures that should map to corresponding structures in
    blinkhash/primitives
...
COIN = 100000000  # 1 blkh in satoshis
...
# Objects that map to blinkhashd objects, which can be serialized/deserialized
...
class CBlockLocator:
    ...
    def serialize(self):
        r = b""
        r += struct.pack("<i", 0)  # Blinkhash Core ignores version field. Set it to 0.
...
class CTransaction:
    ...
    def deserialize(self, f):
        ...
        if len(self.vin) == 0:
            ...
            # matches the implementation in blinkhashd
            if (flags != 0):
                self.vin = deser_vector(f, CTxIn)
...
class CBlockHeader:
    __slots__ = ("hash", "hashMerkleRoot", "hashPrevBlock", "nBits",
                 "auxpow", "nNonce", "nTime", "nVersion", "sha256")
...
class msg_version:
    ...
    def deserialize(self, f):
        ...
        # But, unconditionally check it to match behaviour in blinkhashd
        try:
...
class msg_headers:
    ...
    def deserialize(self, f):
        # comment in blinkhashd indicates these should be deserialized as blocks
