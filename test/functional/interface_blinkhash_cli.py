...
# The block reward of coinbaseoutput.nValue (5000) BLKH/block matures after
# COINBASE_MATURITY (100) blocks. Therefore, after mining 101 blocks we expect
# node 0 to have a balance of (BLOCKS - COINBASE_MATURITY) * 5000 BLKH/block.
BLOCKS = COINBASE_MATURITY + 1
BALANCE = (BLOCKS - 100) * 5000
...
class TestBlinkhashCli(BlinkhashTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("Test -getinfo and blinkhash-cli return all proxies")
        ...
        if self.is_wallet_compiled():
            ...
            assert_equal(Decimal(cli_get_info['Transaction fee rate (-paytxfee) (BLKH/kvB)']), wallet_info['paytxfee'])
            assert_equal(Decimal(cli_get_info['Min tx relay fee rate (BLKH/kvB)']), network_info['relayfee'])
            assert_equal(self.nodes[0].cli.getwalletinfo(), wallet_info)

            # Setup to test -getinfo, -generate, and -rpcwallet= with multiple wallets.
            wallets = [self.default_wallet_name, 'Encrypted', 'secret']
            amounts = [BALANCE + Decimal('4959.999928'), Decimal(9), Decimal(31)]
            ...
            # Mine a block to confirm; adds a block reward (5000 BLKH) to the default wallet.
