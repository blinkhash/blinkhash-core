...
"""Check that it's not possible to start a second blinkhashd instance using the same datadir or wallet."""
import os
import random
import string
...
class FilelockTest(BlinkhashTestFramework):
    ...
    def run_test(self):
        ...
        if self.is_wallet_compiled():
            def check_wallet_filelock(descriptors):
                ...
                self.log.info("Check that we can't start a second blinkhashd instance using the same wallet")
                if descriptors:
